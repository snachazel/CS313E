...
#  Description:A set of recursion exercises using loops, helpers and arrays

#  Student Name:Stpehen Nachazel 
...
def groupSum(start, nums, target):
    #if the target it 0 , you can reach it 
  if target == 0:
    return True 
  #if you are at the end of the list you can't reach the target
  if (start == len(nums)):
    return False
 
  else:
    #if the target is in the list, you can reach the target
    if target == nums[start]:
      return True 
    #otherwise return subtracting the element from the target o not 
    else:
...
def groupSum6(start, nums, target):
  if start == len(nums):
      ...
      #if the element is 6, subtract it from the target 
      if nums[start] == 6:
        return groupSum6(start + 1 , nums , target - nums[start])
      #if the element is not 6 return the results if you subtract it or dont 
      else:
...
def groupNoAdj(start, nums, target):
  #if the target is 0, you can reach it 
  if target == 0:
    return True
  #if you reach the end of the list, you can't reach it 
  if start == len(nums):
    return False 
    ...
    if nums[start] == target:
      return True 
    # if you start at the end of the list, see if the last element is the target
    elif start == len(nums) - 1:
      return (target == nums[start])
    else:
      #return either adding an element or not, depending on the stated conditions 
...
def groupSum5(start, nums, target):
  #if at the end of the list, check if the target is 0 and if not, return false
  if start == len(nums):
    return target == 0
  #recursive case
  else:
    #if the number is a multiple of 5 and not at end of list
    if (nums[start] % 5 )== 0: 
      if (start != (len(nums) - 1)):
        #if the following element is not a one, call function
        if nums[start + 1] != 1:
...
def groupSumClump(start, nums, target):
  #if the target is 0, you can find it
  if target == 0:
    return True
  #if index starts at the end of the list, return false 
  if start == len(nums):
    return False 
  #if there is a consecutive sequence of the same number sum them all
  else:
    sameSum = nums[start]
    count = 1
    for i in range( start + 1 , len(nums)):
      if nums[start] == nums[i]:
        sameSum += nums[i]
        count += 1 
     # return if the consecutive integers reach sum or not 
...
# (No loops needed.)
def splitArray(nums):
  #create two empty lists and sum nums
  b = []
  c = []
  sum_n = sum(nums)
  ...
  result = False
  #if sum nums is twice the sum of a subset, change flag to true
  for element in c:
      if sum_n - element == element:
          result = True
...
def splitArrayHelper(a , b , c , idx):
    #if at end of list, add sum of subarray to list of sums
    if idx == len(a):
        sum_n = sum(b)
        c.append(sum_n)
        return
    #if not at end of list append element to one list or dont
    else:
        d = b[:]
        b.append(a[idx])
...
def splitOdd10Helper(a , b , start):
  #if you start at the end of the list, check if the subarray sum is a multiple of 10, and the other is odd
  if start == len(a):
    return sum(b) % 10 == 0 and ((sum(a) - sum( b)) % 2 == 1)
  else:
    #make a copy of the subarray b 
    c = b[:]
    b.append(a[start])
    return (splitOdd10Helper( a , b , start + 1) or splitOdd10Helper( a , c , start+ 1))
...
def split53(nums):
    #split list into two sums for 3s and 5s
    sum_5 = 0 
    sum_3 = 0 
    return split53Helper(nums , 0 , sum_3 , sum_5)
 

def split53Helper(a , idx , b ,c ):
      #if at the end of the list, see if sums equal
      if idx == len(a):
          return b == c 
     
      else:
          #if element is multiple of 5, add it to 5 sum ,call helper
          if a[idx] % 5 == 0 :
              return split53Helper(a , idx + 1 , b , c + a[idx] )
          #if element is multiple of 5, add it to 5 sum ,call helper
          if a[idx] % 3 == 0 :
              return split53Helper(a , idx + 1 , b + a[idx] , c )
          #all other numbers 
          else:
              return split53Helper(a , idx + 1 , b + a[idx] , c ) or split53Helper(a , idx + 1 , b , c + a[idx] )
